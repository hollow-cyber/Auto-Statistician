...
*   结果可一键下载为纯文本（`.txt` / `.tsv`），方便直接粘贴入 Excel 或 Word。
...
1. **导入数据**：在左侧边栏选择上传 `.txt` 或 `.csv` 文件，或者输入本地文件夹路径直接读取。设定好数据分隔符（逗号、制表符等）。
...
如果您在使用过程中遇到任何 Bug，或有新的功能建议，欢迎提交 Issue，或通过邮件联系技术支持：
📧 **2013850694@qq.com**
